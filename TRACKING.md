...
| 4 | [#886](https://github.com/vincentchalamon/bike-trip-planner/issues/886) | feat(provisioner): zone opening report and manual override import | - | [#952](https://github.com/vincentchalamon/bike-trip-planner/pull/952) | [#884](https://github.com/vincentchalamon/bike-trip-planner/issues/884) |
| 5 | [#891](https://github.com/vincentchalamon/bike-trip-planner/issues/891) | docs(runbooks): zone opening procedure for production and local use | - | [#954](https://github.com/vincentchalamon/bike-trip-planner/pull/954) | [#883](https://github.com/vincentchalamon/bike-trip-planner/issues/883), [#886](https://github.com/vincentchalamon/bike-trip-planner/issues/886) |
...
1. **Stacks à merger d'abord, parent avant enfant** (bases GitHub déjà câblées, `gh stack` #1064/#1065) :
...
| 5 | [#1105](https://github.com/vincentchalamon/bike-trip-planner/issues/1105) | roadbook (05) résumé-seul + détail d'étape (07) catégorisé conformes aux maquettes | L | ✅ [#1114](https://github.com/vincentchalamon/bike-trip-planner/pull/1114) | #1099, #1102, #1103 |
| 6 | [#1106](https://github.com/vincentchalamon/bike-trip-planner/issues/1106) | spike Vulcain (`Fields`/Early-Hints) par-dessus le fetch progressif — **verdict NO-GO** (ADR-057 §4) | S | ✅ [#1115](https://github.com/vincentchalamon/bike-trip-planner/pull/1115) | #1102, #1104 |
...
| Bug | [#1173](https://github.com/vincentchalamon/bike-trip-planner/issues/1173) | require cycle + keyExtractor instable (Math.random) | Task | S | ✅ Mergée ([#1187](https://github.com/vincentchalamon/bike-trip-planner/pull/1187)) |
...
| Perf | [#1194](https://github.com/vincentchalamon/bike-trip-planner/issues/1194) | tree-shake lucide-react-native (suivi #1176) | Task | S | 🚧 En cours ([#1204](https://github.com/vincentchalamon/bike-trip-planner/pull/1204)) |
| Parité | [#1177](https://github.com/vincentchalamon/bike-trip-planner/issues/1177) | consultation in-app d'un voyage partagé /s/&lt;code&gt; | Feature | M | 🚧 En cours ([#1205](https://github.com/vincentchalamon/bike-trip-planner/pull/1205), stack↑#1204) |
| Parité | [#1178](https://github.com/vincentchalamon/bike-trip-planner/issues/1178) | undo/redo (parité web + maquette) | Feature | M | 🚧 En cours ([#1207](https://github.com/vincentchalamon/bike-trip-planner/pull/1207), stack↑#1206) |
| Parité | [#1179](https://github.com/vincentchalamon/bike-trip-planner/issues/1179) | câbler applyBatch (file de modifs) + addPoiWaypoint à l'UI | Task | M | 🚧 En cours ([#1206](https://github.com/vincentchalamon/bike-trip-planner/pull/1206)) |
| Parité | [#1180](https://github.com/vincentchalamon/bike-trip-planner/issues/1180) | onboarding / tour guidé | Feature | M | 🚧 En cours ([#1202](https://github.com/vincentchalamon/bike-trip-planner/pull/1202)) |
...
| Tests | [#1182](https://github.com/vincentchalamon/bike-trip-planner/issues/1182) | couverture jest du flux hébergement manuel #1097 | Task | S | 🚧 Couverture complétée ([#1203](https://github.com/vincentchalamon/bike-trip-planner/pull/1203)) — initiaux #1183 |
...
- **Parité totale avec un hébergement standard** : le manuel est le **même** DTO `Accommodation` / `selectedAccommodation`, seul `source = "manual"` (+ `type = "other"`) le distingue — aucun modèle, type ou composant de rendu parallèle. Indiscernable en aval (budget, export GPX/FIT/texte, partage, carte, persistance, désélection).
- **Custom unique, éphémère, scopé au trip** : persisté uniquement dans le JSONB du `Stage`, jamais écrit en base de référence → jamais proposé à un autre trip/étape ni par un scan. Sélectionné : persiste et se ré-affiche (garde de persistance #870, non écrasé par un rescan incident). Désélectionné : perdu intentionnellement (scan standard, re-saisie manuelle requise, aucun historique).

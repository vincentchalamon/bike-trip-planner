...
- **#888 — différée.** Aucune mesure n'établit l'insuffisance du libellé communal (`admin_level` #880), et DataTourisme porte déjà l'adresse postale sur 121 772 lignes ([`docs/datatourisme-flux-audit.md`](docs/datatourisme-flux-audit.md)). La recommandation 5 de l'audit #879 (`nationalAddressId` vide) est une réserve sur *comment* joindre la BAN, pas une justification de l'importer. À rouvrir si le libellé communal se révèle insuffisant à la mesure.

...
final readonly class OsmOverpassQueryBuilder implements QueryBuilderInterface
{
    ...
    public function buildWaterPointQuery(array $decimatedPoints): string
    {
        ...
        return \sprintf(
            '[out:json][timeout:15];(nwr["amenity"="drinking_water"](around:%d,%s);nwr["amenity"="water_point"](around:%d,%s);nwr["man_made"="water_tap"](around:%d,%s);nwr["natural"="spring"](around:%d,%s););out center;',
            self::AROUND_RADIUS_METERS,
            $polyline,
            self::AROUND_RADIUS_METERS,
            $polyline,

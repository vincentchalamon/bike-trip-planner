...
- **Coverage polygon:** the provisioner materialises `osm.coverage` (union of **every** imported `admin_boundaries` level — country, region, department, commune) inside the staging schema, so it ships atomically with the data; the API tests trip geometry against it via `ST_Covers`. It was the union of `admin_level = 2` alone until issue #880 measured that Geofabrik regional extracts are clipped, so no country relation can be rebuilt from one (0 of 12 on the local set) and the union produced a single `NULL` geometry, i.e. no coverage at all. The levels nest, so unioning all of them uses whatever did build and heals the holes left by a boundary that did not.
...
- The **in-ride assistant** is repointed onto the PostGIS repository (it was a 5-minute Overpass cache). Since [ADR-048](adr-048-in-ride-assistance-without-ai.md) it is AI-free and reads **8 intent buckets across 7 tables** (`InRidePoiRepository`) — the only reader shaped as "radius around a point + tag filters" (KNN `ORDER BY geom <-> point` bounded by `ST_DWithin`), whereas the `api/src/Osm/*Repository` family is corridor-oriented (`ST_DWithin` along the decoded track).

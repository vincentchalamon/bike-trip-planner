...
function SharedTripLoader({ tripId }: { tripId: string }) {
  ...
  useEffect(() => {
    ...
    async function loadSharedTrip() {
      ...
      try {
        const data = await fetchSharedTrip(tripId, token);
  ...
  return (
    <div className="space-y-6">
      ...
      {stages.length > 0 ? (
        ...
        <p className="text-center text-muted-foreground">{t("noStages")}</p>
      )}
    </div>
  );
}
    ...
    <HydrationBoundary>
      <main className="max-w-[1200px] mx-auto px-4 md:px-6 py-8 md:py-12">
        <Suspense fallback={null}>
          <SharedTripLoader tripId={tripId} />
        </Suspense>
      </main>
    </HydrationBoundary>
  );
